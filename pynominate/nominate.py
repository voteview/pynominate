...
def pr_yea(bp, x, w, b):
    pr_y = stats.norm.cdf(dwnominate_Uy(bp, x, w, b) - dwnominate_Un(bp, x, w, b))
    return pr_y


def dwnominate_Uy(bp, x, w, b):
    Uy = b * (
        np.exp(-((x[0] - bp[0] + bp[2]) ** 2 + w * w * (x[1] - bp[1] + bp[3]) ** 2))
    )
    return Uy
...
def update_bp(d, w, b, par0=np.array([0, 0, 0, 0])):
    ...
    return {u"llstart": llstart, u"llend": llend, u"bp": res}
...
def update_idpt(d, w, b, par0=np.array([0, 0, 0, 0])):
    ...
    return {u"llstart": llstart, u"llend": llend, u"x": res}
...
def update_wb(d, start, w0, b0, pool):
    ...
    return {u"llend": res["fun"], u"w": res["x"][0], u"b": res["x"][1]}
        ...
        if "update" in m and m["update"]:
            ...
            if by_congress:
                ...
                for chamber_congress, dat in votes.iteritems():
                    ...
                    chamber = "Senate" if chamber_congress[0] == "S" else "House"
                    ...
                    meta_dict = {
                        ...
                        "number_of_errors": sum(
                            [
                                1
                                if ((0.5 - pr_yea(dat["bp"][:, i], idpt, w, b)) * v) > 0
                                else 0
                                for i, v in enumerate(dat["votes"])
                            ]
                        ),
                    }

                    if congress in ret["idpt"][m["icpsr"]]["meta"]:
                        ret["idpt"][m["icpsr"]]["meta"][congress][chamber] = meta_dict
                    else:
                        ret["idpt"][m["icpsr"]]["meta"][congress] = {chamber: meta_dict}
                ...
                dat = {
                    "votes": np.array(tuple(xx[0] for xx in m["votes"])),
                    "bp": np.transpose(
                        np.array([payload["bp"][str(xx[1])] for xx in m["votes"]])
                    ),
                }

                ret["idpt"][m["icpsr"]]["meta"]["all"]["number_of_errors"] = sum(
                    [
                        1
                        if ((0.5 - pr_yea(dat["bp"][:, i], idpt, w, b)) * v) > 0
                        else 0
            ...
            ret["idpt"][m["icpsr"]]["meta"]["all"][
                ...
                ret["idpt"][m["icpsr"]]["meta"]["all"]["log_likelihood"],
                len(m["votes"]),
            )
            ret["idpt"][m["icpsr"]]["meta"]["all"]["number_of_votes"] = len(m["votes"])
...
def update_nominate(
    ...
    if "votes" in payload:
        nchoices = sum(
            [len([xx for xx in x["votes"] if xx != 0]) for x in payload["votes"]]
        )
    ...
    mapper = pool.map  # allow switching to in/out parallel processing for debugging
    ...
    while iter < maxiter:
        ...
        if "bp" in update:
            ...
            dat = [
                {
                    "votes": np.array(
                        tuple(xx[0] for xx in payload["votes"][i]["votes"])
                    ),
                    "ideal": np.transpose(
                        np.array(
                            [
                ...
                for i in range(len(payload["votes"]))
                if payload["votes"][i]["update"]
            ]
            start = [
                payload["bp"][v["id"]]
                for v in payload["votes"]
            ...
            print(
                "(%03i) Rollcall update data marshal took %2.2f seconds (%i votes)..."
                % (iter + 1, time.time() - starttime, len(start))
            )
            res_bp = mapper(
                update_bp_star, zip(dat, [w] * len(dat), [b] * len(dat), start)
            )
            ...
            print(
                "(%03i) Rollcall update took %2.2f seconds (%i votes)..."
                % (iter + 1, time.time() - starttime, len(start))
            )
        ...
        if "idpt" in update or "bw" in update:
            ...
            dat = [
                {
                    "votes": np.array(
                        tuple(xx[0] for xx in payload["memberwise"][i]["votes"])
                    ),
                    "bp": np.transpose(
                        np.array(
                            [
                ...
                for i in range(len(payload["memberwise"]))
                if payload["memberwise"][i]["update"]
            ]
            start = [
                payload["idpt"][v["icpsr"]]
                for v in payload["memberwise"]
                if v["update"]
            ]
            print(
                "(%03i) Member/BW update data marshal took %2.2f seconds (%i members)..."
                % (iter + 1, time.time() - starttime, len(start))
            )

        if "idpt" in update:
            res_idpt = mapper(
                update_idpt_star, zip(dat, [w] * len(dat), [b] * len(dat), start)
            )
            for i, v in enumerate(payload["memberwise"]):
                if v["update"]:
                    payload["idpt"][v["icpsr"]] = res_idpt[i]["x"]
            print(
                "(%03i) Member update took %2.2f seconds (%i members)..."
                % (iter + 1, time.time() - starttime, len(start))
            )
        ...
        if "bw" in update:
            starttime = time.time()
            start = [payload["idpt"][v["icpsr"]] for v in payload["memberwise"]]
            print(
                "(%03i) Weight and Beta update data marshal took %2.2f seconds (%i members)..."
                % (iter + 1, time.time() - starttime, len(start))
            )
            res_wb = update_wb(dat, start, w, b, mapper)
            w, b = res_wb["w"], res_wb["b"]
            print(
                "(%03i) Weight and Beta  update took %2.2f seconds..."
                % (iter + 1, time.time() - starttime)
            )
            print("\t\t w = %7.4f, b = %7.4f" % (w, b))
            print(
                "(%03i) Iteration Loglik: -%9.4f across %i choices (GMP=%6.4f)\n"
                % (
                    iter + 1,
                    res_wb["llend"],
                    nchoices,
                    np.exp(-res_wb["llend"] / nchoices),
                )
            )
        iter += 1

    print(
        "(XXX) Total update time elapsed %5.2f minutes."
        % ((time.time() - firststarttime) / 60)
    )
    ...
    if "idpt" in update:
        ...
        for i, v in enumerate(payload["memberwise"]):
            if v["update"]:
                ret_idpt[v["icpsr"]] = {
                    "idpt": list(res_idpt[i]["x"]),
                    "meta": {"all": {"log_likelihood": -res_idpt[i]["llend"]}},
                }
        ret["idpt"] = ret_idpt
    if "bw" in update:
        ret["b"] = b
